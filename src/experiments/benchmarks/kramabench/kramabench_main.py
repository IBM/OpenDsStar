"""
KramaBench Experiment Main Entry Point.

This module orchestrates the complete KramaBench experiment:
1. Load KramaBench data using RagDataLoaderFactory
...
class KramaBenchExperiment(BaseExperiment):
    """
    KramaBench experiment implementation.

    This experiment loads KramaBench data, builds retrieval tools from the corpus,
    ...
    def __init__(
        ...
        """
        Initialize KramaBench experiment.
    ...
    def get_data_reader(self) -> DataReader:
        """Get the KramaBench data reader."""
        return KramaBenchDataReader(
    ...
    def get_tools_builder(self) -> Sequence[ToolBuilder]:
        """Get the KramaBench tools builder."""
        return [
    ...
    def get_agent_builder(self) -> AgentBuilder:
        """Get the KramaBench agent builder."""
        return AgentFactory(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run KramaBench experiment. If question_limit is not specified, runs on all questions."
    )
    parser.add_argument(
        "--load-params",
        type=str,
    ...
    if args.load_params:
        ...
        experiment = KramaBenchExperiment.load_instance(Path(args.load_params))
    ...
    combinations = list(product(model_agents, agent_type_enums))
