...
def test_hotpotqa_sample_experiment():
    """
    E2E test: Run HotpotQA experiment on 3 samples and verify tools work properly.
    ...
    2. Validates that tools are properly built and functional:
       ...
       - Vector store tool can search documents
    3. Runs the complete pipeline (data loading, agent execution, evaluation)
    4. Verifies that the average score is above 0.9
    ...
    with patch("src.tools.vector_store_tool.Milvus") as milvus_cls:
        ...
        print(
            f"✓ Data loaded: {len(benchmarks)} questions, {len(corpus)} corpus documents"
        )
        ...
        # HotpotQA uses VectorStoreTool with name "search_hotpotqa"
        assert "search_hotpotqa" in tool_names, "Should have search_hotpotqa tool"

        # Test vector store tool functionality
        vector_store_tool = next(t for t in tools if t.name == "search_hotpotqa")
        print("\nTesting search_hotpotqa tool...")

        # Test search functionality
        search_results = vector_store_tool._run(query="What is the capital?", top_k=3)
        ...
        print(
            f"✓ Vector store search returned {len(search_results)} result(s) (mocked)"
        )
        ...
        # (With mocked vector store, we just verify it runs without errors)
