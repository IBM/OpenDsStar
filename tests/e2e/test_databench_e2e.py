"""End-to-end test for DataBench experiment."""
...
@pytest.mark.e2e
@pytest.mark.slow
def test_databench_sample_experiment():
    """
    E2E test: Run DataBench experiment on 3 samples and verify tools work properly.

    This test:
    1. Creates a DataBench experiment with 3 questions
    ...
    with patch(
        ...
        assert (
            config["code_mode"] == "stepwise"
        ), "DataBench experiment should default to stepwise code mode"

        # ===== TOOL VALIDATION SECTION =====
        # This section validates that tools are properly built before running the experiment
        print("\n" + "=" * 60)
        ...
        # description should encourage reviewing all summaries
        assert (
            "review" in search_tool.description.lower()
        ), "Description should mention reviewing results"
        ...
        outputs, results = experiment.experiment_main(
            run_id="e2e_test_databench_3samples",
            fail_fast=False,
        )
        ...
        # (DataBench is challenging, so we just verify it runs without errors)
